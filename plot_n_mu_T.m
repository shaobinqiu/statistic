function plot_n_mu_T(n_mu_T, mu, T,A, B)
figure
set(gcf,'color','white');
n_mu_T= imrotate(n_mu_T,90);
image(n_mu_T,'CDataMapping','scaled')
title(['n_{', A,'}(\Delta\mu, T)'])
xlabel(['\mu_{', A,'}-\mu_{', B,'}(eV)'])
ylabel('Temperature(K)')
colorbar
n_x=5;n_y=5;%%%number of axis label
xl={};yl={};
for ii=1:n_x
    xl=[xl num2str(min(mu(end,:))+(ii-1)*(max(mu(end,:))-min(mu(end,:)))/(n_x-1))]; 
end
for ii=1:n_y
...
line_n=[0.05:0.1:0.95];%%%%%%plot the line of n=line_n(ii)
